...
import { ChevronDown, Plus, X, Flame, Zap } from "lucide-react";
...
function ProgressPage() {
    ...
    const load = async () => {
      ...
      const [wRes, fRes, hRes, sRes] = await Promise.all([
        supabase.from("weight_logs").select("*").eq("user_id", user.id)
          .order("logged_at", { ascending: true }).limit(120),
        supabase.from("food_logs").select("date,calories,protein,carbs,fat,logged_at")
          .eq("user_id", user.id).gte("date", startISO).lte("date", endISO),
        supabase.from("water_logs").select("date,amount_ml")
          .eq("user_id", user.id).gte("date", startISO).lte("date", endISO),
  ...
  const compliantDays = dayTotals.filter((d, i) => {
    const isFuture = weekDays[i].getTime() > Date.now();
    if (isFuture || d.calories === 0) return false;
    return Math.abs(d.calories - dailyTarget) <= dailyTarget * 0.1;
  }).length;
  const elapsedDays = weekDays.filter((d, i) => d.getTime() <= Date.now() && dayTotals[i].calories > 0).length || 1;
  const compliancePct = Math.round((compliantDays / Math.max(elapsedDays, 1)) * 100);

  // ── Life Score (0-150) ─────────────────────────
  const proteinTarget = profile?.protein_g ?? 100;
  const waterByDay = weekDays.map((d) => {
    const iso = isoDate(d);
    return waters.filter((w) => w.date === iso).reduce((a, w) => a + Number(w.amount_ml ?? 0), 0);
  });
  ...
  const lifeScore = Math.min(
    150,
    Math.round(compliantDays * 12 + proteinDays * 6 + hydrationDays * 4 + Math.min(streak?.current_streak ?? 0, 14) * 1.5),
  );
  const lastWeekScore = Math.max(0, lifeScore - 11); // mocked diff
  const scoreDelta = lifeScore - lastWeekScore;
  ...
  const journeyPct = startWeight != null && targetWeight != null && currentWeight != null
    ? Math.max(0, Math.min(100, ((Number(startWeight) - Number(currentWeight)) / (Number(startWeight) - Number(targetWeight) || 1)) * 100))
    : 0;
  ...
  const logWeight = async () => {
    ...
    if (error) { toast.error(error.message); }
      ...
      const { data } = await supabase.from("weight_logs").select("*")
        .eq("user_id", user.id).order("logged_at", { ascending: true }).limit(120);
      setWeights(data ?? []);
  ...
  // ── Insights (local, deterministic) ──────────
  const insights = buildInsights({
    proteinDays,
    hydrationDays,
    avgWater: waterByDay.reduce((a, b) => a + b, 0) / 7 / 250,
    eveningPct: computeEveningPct(foods),
    compliantDays,
    streak: streak?.current_streak ?? 0,
  });

  return (
    <div className="px-5 pt-7 pb-6 stagger">
      {/* Header */}
      ...
      {/* Life Score */}
      <div className="mt-5 rounded-[28px] p-7 bg-gradient-hero text-center text-white shadow-elev-md">
        <div className="text-[11px] tracking-[0.2em] uppercase font-semibold text-[color:var(--gold)]">
          Weekly Life Score
        </div>
        ...
        <div className="text-[13px] text-white/70 mt-1">
          {scoreDelta >= 0 ? "↑" : "↓"} {Math.abs(scoreDelta)} points from last week
        </div>
        <button className="text-[13px] text-[color:var(--gold)] mt-3 font-medium">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        ...
        >
          ...
          <div className="font-display text-[32px] font-bold text-[color:var(--gold)] leading-none mt-2">
            {streak?.current_streak ?? 0}<span className="text-[18px] font-medium ml-1">d</span>
          </div>
          <div className="text-[13px] text-[color:var(--ink-mid)] mt-1">Current streak</div>
          <div className="text-[12px] text-[color:var(--ink-light)] mt-0.5">
        ...
        >
          ...
          <div className="font-display text-[32px] font-bold text-[color:var(--forest)] leading-none mt-2">
            {compliancePct}<span className="text-[18px] font-medium">%</span>
          </div>
          <div className="text-[13px] text-[color:var(--ink-mid)] mt-1">Weekly compliance</div>
          <div className="text-[12px] text-[color:var(--ink-light)] mt-0.5">Days you hit your target</div>
        </div>
      </div>

      {/* Calorie chart */}
      <div className="mt-4 bg-white rounded-[20px] border border-[color:var(--cream-border)] p-5 shadow-elev-sm">
      ...
      {/* Macro trends */}
      <div className="mt-4 bg-white rounded-[20px] border border-[color:var(--cream-border)] p-5 shadow-elev-sm">
        <h4 className="font-body font-semibold text-[14px] mb-3">Macro trends</h4>
      ...
      <div className="mt-4 bg-white rounded-[20px] border border-[color:var(--cream-border)] p-5 shadow-elev-sm">
        ...
        {weights.length >= 2 ? (
            ...
            {targetWeight && (
              <div className="mt-3">
                <div className="h-2 rounded-full bg-[color:var(--cream-dark)] overflow-hidden">
                  <div
                    className="h-full bg-gradient-cta transition-all duration-700"
          ...
          <div className="py-6 text-center">
            <p className="text-[14px] text-[color:var(--ink-mid)]">Logged: <strong>{Number(weights[0].weight_kg).toFixed(1)} kg</strong></p>
            <p className="text-[12px] text-[color:var(--ink-light)] mt-1">One more entry to see your trend.</p>
          </div>
        ) : (
      ...
      {/* Insights */}
      <div className="mt-5">
        <h4 className="font-body font-semibold text-[14px] mb-3">Your insights this week</h4>
        <div className="space-y-2.5">
          {insights.map((ins, i) => (
            <div
              key={i}
              className="bg-white rounded-[14px] p-3.5 border border-[color:var(--cream-border)] flex gap-3"
              style={{ borderLeft: `4px solid ${ins.color}` }}
            >
              <span className="text-[20px] leading-none mt-0.5">{ins.icon}</span>
              <p className="text-[13px] text-[color:var(--ink-mid)] leading-snug">
                <span className="font-semibold text-[color:var(--ink)]">{ins.title}: </span>
                {ins.body}
              </p>
            </div>
          ))}
      ...
      {showWeightModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 animate-fade-in" onClick={() => setShowWeightModal(false)}>
          <div
            className="w-full max-w-[430px] bg-white rounded-t-[28px] p-6 pb-8 animate-fade-up"
          ...
          >
            ...
            <h3 className="font-display text-[24px] font-bold text-[color:var(--ink)] mt-1">
            ...
            <div className="mt-6 flex items-end justify-center gap-2">
              <input
                ...
                placeholder="0.0"
                className="w-[180px] text-center font-display text-[56px] font-bold text-[color:var(--forest)] bg-transparent outline-none border-b-2 border-[color:var(--cream-border)] focus:border-[color:var(--forest)]"
              />
...
function SemicircleGauge({ score, max }: { score: number; max: number }) {
  ...
  // Build segment paths in degrees: 180° (left) to 360°/0° (right)
  // Mapping: 0°→180°left, 180°→right
  const segments = [
    { from: 0, to: 50 / max, color: "#E8624A" },
    { from: 50 / max, to: 100 / max, color: "#C4973A" },
    { from: 100 / max, to: 130 / max, color: "#3D7A58" },
    { from: 130 / max, to: 1, color: "#F5C85C" },
  ];

  const angleAt = (t: number) => Math.PI + Math.PI * t; // t in [0,1]
  const point = (t: number) => ({
    x: cx + r * Math.cos(angleAt(t)),
    y: cy + r * Math.sin(angleAt(t)),
  });
  ...
  const ptr = point(t);
  // pointer triangle perpendicular
  const a = angleAt(t);
  ...
  return (
    <div className="flex flex-col items-center mt-1">
      <svg viewBox={`0 0 ${w} ${h}`} className="w-[260px] h-[140px]">
        ...
        {/* Pointer */}
        <polygon
          points={`${inner.x},${inner.y} ${left.x},${left.y} ${outer.x},${outer.y} ${right.x},${right.y}`}
          fill="white"
          stroke="white"
        />
        <circle cx={ptr.x} cy={ptr.y} r={3} fill="white" />
      </svg>
...
function CalorieBars({
  ...
  setActiveBar: (n: number | null) => void;
}) {
  const max = Math.max(target * 1.2, ...totals.map((t) => t.calories), 1);
    ...
    <div className="mt-4">
      <div className="relative h-[140px] flex items-end justify-between gap-2 px-1">
        ...
        <div
          ...
          style={{ borderColor: "var(--gold)", top: `${targetY * 100}%` }}
        >
          <span
            className="absolute right-0 -top-5 text-[10px] uppercase tracking-wider font-semibold"
          ...
          return (
            ...
            >
              ...
              <div
                ...
                style={{
                  height: `${Math.max(heightPct, t.calories > 0 ? 4 : 0)}%`,
                  background: over ? "var(--coral)" : "var(--forest)",
                  opacity: t.calories === 0 ? 0.18 : 1,
                  boxShadow: isActive ? "0 0 0 2px var(--cream)" : undefined,
                }}
              />
              <span
                className={`text-[11px] mt-2 ${
                  isToday ? "text-[color:var(--forest)] font-bold" : "text-[color:var(--ink-light)] font-medium"
                }`}
              >
                {labels[i]}
              </span>
...
function MacroLine({
  label, unit, color, target, values,
}: { label: string; unit: string; color: string; target: number; values: number[] }) {
  const w = 280;
  const h = 44;
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="w-14">
        ...
        <div className="text-[12px] text-[color:var(--ink-mid)] font-semibold">
          {Math.round(today)}<span className="text-[10px] text-[color:var(--ink-light)]">{unit}</span>
        </div>
      </div>
      <svg viewBox={`0 0 ${w} ${h}`} className="flex-1 h-11">
        ...
        {values.map((v, i) => (
          <circle key={i} cx={i * stepX} cy={h - (v / max) * (h - 6) - 3} r={v > 0 ? 2.5 : 0} fill={color} />
        ))}
      </svg>
      <div className="text-[10px] text-[color:var(--ink-light)] w-10 text-right">
        /{target}{unit}
      </div>
    </div>
  );
  ...
  return (
    <div className="mt-4 -mx-1">
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-28">
        ...
        {targetY != null && (
          <line x1={0} x2={w} y1={targetY} y2={targetY} stroke="var(--gold)" strokeDasharray="4 4" strokeWidth="1.5" />
        )}
        ...
        {weights.map((v, i) => (
          <circle key={i} cx={i * stepX} cy={h - ((v - min) / range) * (h - 12) - 6} r={3} fill="white" stroke="var(--forest-mid)" strokeWidth="2" />
        ))}
      </svg>
    </div>
  );
...
function buildInsights(d: {
  proteinDays: number; hydrationDays: number; avgWater: number;
  eveningPct: number; compliantDays: number; streak: number;
}): Insight[] {
  const out: Insight[] = [];
  out.push({
    icon: "💪",
    title: "Protein goals",
    body: `You hit your protein target ${d.proteinDays}/7 days this week — ${d.proteinDays >= 5 ? "great for muscle maintenance!" : "a few more days will boost recovery."}`,
    color: "#7C5CCC",
  });
  if (d.eveningPct > 0) {
    out.push({
      ...
      body: `${d.eveningPct}% of your calories were eaten after 6pm. ${d.eveningPct > 50 ? "Try shifting lunch to be larger." : "Nice balance across the day."}`,
      color: "#3B82B8",
    });
  }
  out.push({
    icon: "💧",
    title: "Hydration",
    body: `You averaged ${d.avgWater.toFixed(1)} glasses/day — ${d.avgWater >= 8 ? "right on target!" : "slightly below your 8-glass goal."}`,
    color: "#3D9FD9",
  });
  if (d.streak >= 3) {
    out.push({
      ...
      body: `${d.streak}-day logging streak. Showing up daily is the #1 predictor of long-term results.`,
      color: "#C4973A",
    });
  } else {
    out.push({
      icon: "🎯",
      title: "Compliance",
      body: `You hit your calorie target on ${d.compliantDays} day${d.compliantDays === 1 ? "" : "s"} this week. Aim for 5+ to accelerate progress.`,
      color: "#3D7A58",
    });
  }
  return out;
